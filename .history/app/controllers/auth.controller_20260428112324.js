...
/**
 * 🔹 REFRESH TOKEN
 * Genera un nuevo accessToken usando el refreshToken
 */
export const refresh = async (req, res) => {
  try {
    const { refreshToken } = req.body;
    if (!refreshToken) return errorResponse(res, 403, "Refresh token requerido");

    const stored = await RefreshToken.findOne({ where: { token: refreshToken } });
    if (!stored) return errorResponse(res, 403, "Refresh token inválido o eliminado");

    if (new Date() > stored.expiresAt) {
      await stored.destroy();
      return errorResponse(res, 403, "Refresh token expirado. Inicia sesión de nuevo.");
    }
...
/**
 * 🔹 LOGOUT REAL
 * Elimina el refreshToken de la BD
 */
export const logout = async (req, res) => {
  try {
    const { refreshToken } = req.body;
    if (!refreshToken) return errorResponse(res, 400, "Refresh token requerido");

    const deleted = await RefreshToken.destroy({ where: { token: refreshToken } });
    if (!deleted) return errorResponse(res, 404, "Token no encontrado");
